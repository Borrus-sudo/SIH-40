...
const Heatmap = ({
    ...
    useEffect(() => {
        ...
        // Bin data
        const xBinsArr = d3.bin().domain(xScale.domain()).thresholds(xBins)(
            data.map((d) => d[xField])
        );
        const yBinsArr = d3.bin().domain(yScale.domain()).thresholds(yBins)(
            data.map((d) => d[yField])
        );

        const freqMap = {};
        data.forEach((d) => {
            const xBin = xBinsArr.findIndex(
                (b) => d[xField] >= b.x0 && d[xField] < b.x1
            );
            const yBin = yBinsArr.findIndex(
                (b) => d[yField] >= b.x0 && d[yField] < b.x1
            );
            const key = `${xBin}-${yBin}`;
