...
function Dashboard() {
    const tempData = Array.from({ length: 20 }, (_, i) => ({ x: i * 5, y: Math.random() * 25 }));
    const salData = Array.from({ length: 20 }, (_, i) => ({ x: i * 5, y: Math.random() * 40 }));
    const presData = Array.from({ length: 20 }, (_, i) => ({ x: i * 5, y: Math.random() * 1000 }));
